...
func (this *Router) changeMethod(origin *routerFactoryHandler) routerHandler {
	...
	if origin.urlPrefixHandler != nil {
		...
		for seg, singleUrlPrefixHandler := range origin.urlPrefixHandler {
			...
			sort.SliceStable(param, func(left int, right int) bool {
				return param[left].index < param[right].index
			})
		...
		sort.SliceStable(resultUrlPrefixHandler, func(left int, right int) bool {
			return resultUrlPrefixHandler[left].segment < resultUrlPrefixHandler[right].segment
		})
		result.urlPrefixHandler = resultUrlPrefixHandler
	}
